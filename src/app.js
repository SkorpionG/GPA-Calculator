...
function updateContainerHeight() {
    ...
    const visibleEntry = document.querySelector(".subject-entry:not(.subject-entry-template)");
    if (!visibleEntry) return;

    // 計算實際高度（包含 margin）
    const style = window.getComputedStyle(visibleEntry);
    const height =
        visibleEntry.offsetHeight + parseFloat(style.marginTop) + parseFloat(style.marginBottom);
